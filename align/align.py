...
try:
    mkdir(f'../outputs/aligned/{argv[1]}')
except:
    pass

for dir in listdir(f'../outputs/detected/{argv[1]}'):
    try:
        mkdir(f'../outputs/aligned/{argv[1]}/{dir}')
    except:
        pass
    for file in listdir(f'../outputs/detected/{argv[1]}/{dir}'):
    # reading image
        img = cv2.imread(f'../outputs/detected/{argv[1]}/{dir}/{file}')
        img_raw = img.copy()
