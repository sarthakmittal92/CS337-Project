...
try:
    mkdir(f'../outputs/detected/{argv[1]}')
except:
    pass

for dir in listdir(f'../{argv[1]}'):
    try:
        mkdir(f'../outputs/detected/{argv[1]}/{dir}')
    except:
        pass
    for file in listdir(f'../{argv[1]}/{dir}'):
        # image resizing
        ...
        for i, instance in detections_df.iterrows():
            ...
            idx += 1
            try:
                cv2.imwrite(f'../outputs/detected/{argv[1]}/{dir}/detected_{file.split(".")[0]}.png',detected_face[:,:,::-1])
            except:
                pass